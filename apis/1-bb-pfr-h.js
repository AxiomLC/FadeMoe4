// SCRIPT: bb-pfr-h.js (Unified Premium Funding Rate Backfill + Streamlined Status Logging + ANSI Color)
// Updated: 14 Oct 2025
...
// ============================================================================
// Tracks completion status for each perpspec
  ...
  const heartbeatId = setInterval(async () => {
    for (const [key, cfg] of Object.entries(EXCHANGES)) {
      if (!completedPerpspecs.has(cfg.perpspec)) {
        ...
        const runningMessage = errSet.size > 0 ? `${cfg.perpspec} 429 errors on ${errSet.size} symbols.` : `${cfg.perpspec} backfilling db.`;
