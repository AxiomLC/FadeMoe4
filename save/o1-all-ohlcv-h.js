...
//*********************************************see below eight monitor line */
    try {
      const response = await axios.get(config.url, { params, timeout: config.timeout });
      weightMonitor.logRequest('bin-ohlcv', '/fapi/v1/klines', 1); // ***Added by Claude AI 13 Oct
...
async function createMTTokenFinalLoop() {
  try {
    ...
    const now = Date.now();
    const startTs = now - (4 * 60 * 1000); // hardcoded to 4 , to finich script
