...
async function backfill() {
  ...
  // NEW 30-SECOND HEARTBEAT LOGIC: Logs status every 30 seconds for all running exchanges
  ...
  const heartbeatInterval = setInterval(async () => {
    ...
    for (const exKey of Object.keys(EXCHANGES)) {
        ...
        // Ensures the full requested status string is used for both console and DB message field
        const statusMessage = `${config.perpspec} backfilling db."`;
        
        // Log to DB (status field is 'running', message field is the full string)
        await apiUtils.logScriptStatus(
            dbManager,
            SCRIPT_NAME,
            'running',
