...
const STATUS_LOG_COLOR = '\x1b[35m'; // Purple
...
const symbolsToProcess = perpList.includes('MT') ? perpList : [...perpList, 'MT'];
// ============================================================================
// UTILITIES
// ============================================================================
function sleep(ms) {
  ...
  for (const exchange of EXCHANGES) {
    ...
    try {
      ...
      return {
        ...
        oi: isMT ? null : (row.oi ? parseFloat(row.oi) : null),
        pfr: isMT ? null : (row.pfr ? parseFloat(row.pfr) : null),
        lsr: isMT ? null : (row.lsr ? parseFloat(row.lsr) : null),
        lql: isMT ? null : (row.lql ? parseFloat(row.lql) : null),
        lqs: isMT ? null : (row.lqs ? parseFloat(row.lqs) : null),
        rsi1: row.rsi1 ? parseFloat(row.rsi1) : null,
        rsi60: row.rsi60 ? parseFloat(row.rsi60) : null,
        tbv: isMT ? null : (row.tbv ? parseFloat(row.tbv) : null),
        tsv: isMT ? null : (row.tsv ? parseFloat(row.tsv) : null)
      };
    }));
    } catch (error) {
