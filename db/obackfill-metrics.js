// ============================================================================
// BACKFILL METRICS MODULE  backfill-metrics.js  22 Oct 2025 (UNIFIED SCHEMA)
// Backfills perp_metrics from perp_data using ON CONFLICT DO UPDATE (full overwrite for derived metrics)
...
const BATCH_SIZE = 1000;             // For any sub-batches if needed
...
async function getBackfillStartTs(symbol, exchange, retentionStart) {
  ...
  `;
  try {
    const result = await dbManager.query(query, [symbol, exchange]);
...
// ============================================================================
// FETCH AND AGGREGATE DATA FOR BACKFILL (UNIFIED SCHEMA)
// ============================================================================
async function fetchAndAggregateDataForBackfill(symbol, exchange, startTs, endTs) {
  ...
  // Single query per exchange (unified: all fields in range)
  const baseQuery = `
  ...
  `;
  try {
    const result = await dbManager.query(baseQuery, [symbol, exchange, BigInt(bufferedStart), BigInt(endTs)]);
    let allData = result.rows.map(row => ({
      ts: Number(row.ts),
      symbol: row.symbol,
    ...
    // Group and merge by ts/exchange (unified: check field presence)
    const grouped = {};
    allData.forEach(row => {
      const ts = row.ts;
      if (!grouped[ts]) {
    ...
    console.error(`Error fetching data for backfill ${symbol} ${exchange}:`, error.message);
    return [];
  }
}
...
async function backfillSymbolExchange(symbol, exchange, retentionStart, endTs) {
  try {
    ...
    return { records: metrics.length, metrics, skipped: false }; // Return metrics for global collect
  } catch (error) {
    ...
    return { records: 0, metrics: [], skipped: false, error: true };
  }
}

// ============================================================================
// MAIN BACKFILL FUNCTION
// Parallel processing with heartbeat progress; global insert for efficiency
// ============================================================================
async function runBackfill() {
  ...
  try {
    ...
    let allMetrics = []; // Global collection

    // Heartbeat for progress
    const heartbeatInterval = setInterval(() => {
      const progress = ((processedCount / totalTasks) * 100).toFixed(1);
      logStatus('running', 
        `Backfilling: ${processedCount}/${totalTasks} (${progress}%) - ${totalRecords} records queued`);
    ...
    // Sort global metrics (ts asc, symbol asc, exchange asc)
    allMetrics.sort((a, b) => {
    ...
    // Single global insert
    let globalInserted = 0;
    if (allMetrics.length > 0) {
      const globalInsertResult = await dbManager.insertMetrics(allMetrics);
      globalInserted = globalInsertResult.rowCount || allMetrics.length;
      totalRecords = globalInserted; // Override with actual
    }

    // Final summary
    const duration = ((Date.now() - startTime) / 1000).toFixed(2);
    await logStatus('completed', 
      `Backfill complete: ${globalInserted} records inserted, ${skippedCount} skipped, ${errorCount} errors in ${duration}s`);
    ...
    return { recordsInserted: globalInserted, tasksProcessed: processedCount, errors: errorCount };
