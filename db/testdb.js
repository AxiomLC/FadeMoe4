...
async function fetchRawData(symbol, exchange, startTs, endTs) {
  ...
  return result.rows.map(row => ({
    ...
    oi: row.oi !== null ? parseFloat(row.oi) : null,
    pfr: row.pfr !== null ? parseFloat(row.pfr) : null,
    lsr: row.lsr !== null ? parseFloat(row.lsr) : null,
    lql: row.lql !== null ? parseFloat(row.lql) : null,
    lqs: row.lqs !== null ? parseFloat(row.lqs) : null,
...
async function runTestBackfill() {
  ...
  try {
    const tasks = perpList.map(symbol => limit(async () => {
